...
never *create or widen* it. Before any payment reaches a gateway, the Passport runs eleven
...
- **Nothing in the enforcement path reads the text an attacker can manipulate.** Every
  ...
  `docs/results.json`.
- **We ran our own adversarial pass against the running system and fixed what it found.**
  Two gaps surfaced live — no check tied a mandate to the agent it was issued to, and
  `apps/agent`'s process environment held the Razorpay credentials it never used. Both are
  closed: check 11 (`apps/passport/src/checks/11-mandate-agent.ts`, `MANDATE_AGENT_MISMATCH`)
  and a per-service `apps/agent/.env` that structurally cannot contain
  `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET`. See `docs/JUDGE-QA.md` Q11/Q12 for the full
  before-and-after, including the live test that caught each one.
...
Policy matrix: 14/14 cases pass (`tests/policy.spec.ts`). Concurrency: 10/10 rounds
...
- Judge Q&A, including two gaps a live adversarial pass found — and this project then fixed: [JUDGE-QA.md](JUDGE-QA.md)
